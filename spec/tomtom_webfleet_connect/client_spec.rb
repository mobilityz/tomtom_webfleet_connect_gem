...
describe TomtomWebfleetConnect::Client do
  ...
  describe 'attributes' do
    ...
    before do
      @tomtom_webfleet_connect = TomtomWebfleetConnect::Client.new
    end

    it 'have no API by default' do
      expect(@tomtom_webfleet_connect.api_key).to be_nil
    end
    ...
    it 'set an API key, account, user and password in constructor' do
      tomtom_webfleet_connect = TomtomWebfleetConnect::Client.new(api_key: @api_key, account: @account, user: user.name, mdp: user.password)
      expect(tomtom_webfleet_connect.api_key).to eq(@api_key)
      expect(tomtom_webfleet_connect.account).to eq(@account)
    end

    it 'set an API key via setter' do
      @tomtom_webfleet_connect.api_key = @api_key
      expect(@tomtom_webfleet_connect.api_key).to eq(@api_key)
    end
  ...
  describe 'method' do
    ...
    before do
      @parameters_url = @root_url + "?account=#{@account}&apikey=#{@api_key}&lang=#{@lang}&useISO8601=#{@use_ISO8601}&useUTF8=#{@use_UTF8}&outputformat=#{@outputformat}"
    ...
    it 'get root API url' do
      expect(client.endpoint).to eq(@root_url)
    end
    ...
    it 'with action return one line' do
      ...
      expect(response.error).to eq(false)
      expect(response.success).to eq(true)
      expect(response.response_body).not_to be_nil
    end

    xit 'with action return hash' do
